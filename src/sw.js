 ...
 *
 * Offline behavior, for future reference:
 * - precacheAndRoute() (below) makes every build asset — including
 *   index.html — available via Workbox's precache. Its route is matched
 *   BEFORE the NetworkFirst route further down, and its handler checks the
 *   cache first and only touches the network if the entry is missing. So
 *   once install has completed once, reloading "/" (or the manifest's
 *   start_url) never depends on the network again — Workbox's directoryIndex
 *   handling maps "/" to the cached "index.html" entry automatically.
 * - That means the app shell is already fully offline-capable on its own.
 *   The NetworkFirst route below only ever runs for a same-origin GET that
 *   ISN'T in the precache manifest (effectively a safety net for anything
 *   added at runtime), so it's given a short timeout so a dead/slow network
 *   can't stall it — it should fall back to cache almost immediately.
 */
...
// Bump this whenever shell offline behavior changes, so activate() purges any
// stale runtime-cache entries left by a previous version.
...
// Safety-net strategy for any same-origin GET that isn't in the precache
// manifest (see comment at top of file). Bounded to a short timeout so a
// slow or dead connection can't stall the response — it falls back to
// whatever's cached almost immediately instead of hanging.
registerRoute(
